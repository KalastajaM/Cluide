...
data/
  ...
  envelope_balances.json    ← refreshed by Pattern 3 (browser extraction)
...
```
Script: extract_envelopes.js  (run in browser console on your budget app's accounts page)
Target: envelope balances and account balances
Output: JSON copied to clipboard
Save to: data/envelope_balances.json
```

Example extraction script:

```javascript
// Run in browser console on the budget app's accounts page
const envelopes = [];
...
```markdown
## Refresh envelope balances (manual, ~2 minutes)
When: before running a spending analysis, or weekly
Steps:
  1. Open your budget app in Chrome, navigate to Accounts
  2. Open console (Cmd+Option+J), paste contents of scripts/extract_envelopes.js
  3. Press Enter — JSON is copied to clipboard
  4. Open data/envelope_balances.json and replace the contents
...
def extract_from_screenshot(image_path: Path) -> list[dict]:
    ...
    if response.stop_reason == "max_tokens":
        print(f"WARNING: {image_path.name}: response truncated — skipping")
        return []
    try:
        transactions = json.loads(response.content[0].text)
    except json.JSONDecodeError as e:
        print(f"WARNING: {image_path.name}: malformed JSON ({e}) — skipping")
        return []
    if not isinstance(transactions, list):
        print(f"WARNING: {image_path.name}: expected a JSON array — skipping")
        return []
    return transactions

...
└── LESSONS.md            ← Append-only reasoning history (load on demand)
```

Create `TASK_REFERENCE.md` (schemas, format templates, edge-case rules — loaded on demand) when TASK.md approaches its 250-line cap.
...
Append to `RUN_LOG.md`. Keep the last 20 entries in full; once the file exceeds ~30 entries, archive older ones to `RUN_LOG_ARCHIVE.md`.

# [PLACEHOLDER: Initiative Name] – Knowledge Base

**Owner:** [PLACEHOLDER: Initiative Owner Name] | **Sponsor:** [PLACEHOLDER: Workstream Sponsor Name]
**Last updated:** [PLACEHOLDER: YYYY-MM-DD] (rev 1 — initial template)

**Purpose:** The central institutional memory for this initiative. Capture new insights, field intelligence, findings, decisions, and any relevant information here. When in doubt about where to record something, put it here.

> **Note on duplication:** Formal decisions belong in `Decision_Tracker.md`. Risks belong in `Risk_Register.md`. Dependencies belong in `Dependency_Register.md`. Open actions belong in `Action_Tracker.md`. Use this file for everything that doesn't fit neatly into those registers — context, analysis, field intel, stakeholder notes, working assumptions, and emerging patterns.

---

## How to use this file

- Add new entries at the **top** of the relevant section, or create a new section if needed.
- Each entry should include a date, a brief title, and the content.
- Link to register entries (R-xx, D-xx, ACT-xx, DEC-xx) where relevant.
- Older or superseded entries can be marked ~~strikethrough~~ but should not be deleted — they provide historical context.

---

## Section index

1. [Migration Execution](#1-migration-execution)
2. [Product Gaps & Technical Blockers](#2-product-gaps--technical-blockers)
3. [Customer Segmentation & Account Mapping](#3-customer-segmentation--account-mapping)
4. [Partner Dynamics](#4-partner-dynamics)
5. [Financial Model & Data](#5-financial-model--data)
6. [Organisational & Incentives](#6-organisational--incentives)
7. [Regulatory & Legal](#7-regulatory--legal)
8. [Stakeholder Notes](#8-stakeholder-notes)
9. [Working Assumptions](#9-working-assumptions)

---

## 1. Migration Execution

*[PLACEHOLDER: Add insights about how migrations are actually running — what's working, what's not, field feedback from the migration team, conversion velocity observations, etc.]*

---

## 2. Product Gaps & Technical Blockers

*[PLACEHOLDER: Add findings about product gaps blocking migration — feature readiness, R&D timelines, workarounds discovered in the field, customer feedback on specific technical issues. Cross-reference with `Migrations/Product_Dependencies.md` for formal tracking.]*

---

## 3. Customer Segmentation & Account Mapping

*[PLACEHOLDER: Add intelligence about the customer base — air-gapped account estimates, high-touch vs tech-touch segment refinements, governmental/regulated account characteristics, account concentration data, etc.]*

---

## 4. Partner Dynamics

*[PLACEHOLDER: Add field intelligence about partner behaviour — resistance patterns, which partners are cooperating, managed service revenue dependencies, partner tier implications of the programme, top partner account plan status, etc.]*

---

## 5. Financial Model & Data

*[PLACEHOLDER: Add notes on financial model assumptions, data quality issues discovered, model updates made and why, stress-test results, scenario analysis findings, etc.]*

---

## 6. Organisational & Incentives

*[PLACEHOLDER: Add notes on operating model discussions, coverage gaps, incentive plan status, handoff planning from migration team to local teams, resourcing constraints, etc.]*

---

## 7. Regulatory & Legal

*[PLACEHOLDER: Add notes on regulatory findings — which frameworks create cloud prohibitions, how many accounts are affected, legal opinions obtained, any emerging compliance risks, etc.]*

---

## 8. Stakeholder Notes

*[PLACEHOLDER: Add notes from key stakeholder conversations — alignment status, concerns raised, commitments made, escalations, etc. Note: do not record personally sensitive information here.]*

---

## 9. Working Assumptions

*[PLACEHOLDER: Document assumptions that underpin the plan but have not yet been formally validated. Flag these explicitly so they can be tracked and confirmed or revised over time.]*

| # | Assumption | Status | Date added | Date validated / revised |
|---|---|---|---|---|
| 1 | [PLACEHOLDER: e.g. "Customers require 3–6 months for migration planning and execution"] | Working | [PLACEHOLDER: Date] | — |
| 2 | [PLACEHOLDER: e.g. "Gross margin for legacy product is approximately X%"] | Working | [PLACEHOLDER: Date] | — |
| 3 | [PLACEHOLDER: e.g. "Non-migratable base is estimated at X% of total ACV"] | Unvalidated | [PLACEHOLDER: Date] | — |

---

*This knowledge base is a living document. Populate it continuously as the programme progresses.*

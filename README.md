...
This repository now has shared contributor instructions in [AGENTS.md](./AGENTS.md), a Claude entry point in [CLAUDE.md](./CLAUDE.md), and [setup instructions for Claude, ChatGPT, and Codex](./PLATFORM_SETUP.md). For your own project, start with [the quickstart](./00_QUICKSTART.md) or [Guide 35](./35_DUAL_PLATFORM_PROJECTS.md). Claude-specific hooks and agent definitions remain labelled; they are not installed on OpenAI by copying their folders.
...
Cluide began as a Claude-built guide, drawing on the maintainer’s hands-on experience. It is now maintained with Claude and ChatGPT/Codex and covers both platforms. The original authorship history remains part of the project; current instructions distinguish the products instead of assuming their capabilities match.
...
| Your surface | Start here |
|---|---|
| Claude Code or Codex | Open a local project folder; create shared policy and native entry points using the quickstart. |
| ChatGPT project or conversational Claude | Supply the policy and needed workflow files as sources, set the project bootstrap, and verify source freshness. |
| Both platforms | Use Guide 35 and `tasks/setup-dual-platform.md`; choose one scheduler owner per job. |


**New here?** Start with [00_QUICKSTART.md](./00_QUICKSTART.md) — build shared instructions and a planning workflow on your chosen surface.
...
| 05 | [MCP Servers](./05_MCP_SERVERS.md) | All | Connecting your assistant to Gmail, Calendar, GitHub, etc. |
...
| 13 | [Dev & Execution Workflow](./13_DEV_EXECUTION_WORKFLOW.md) | Power users | Build vs. run across Claude and OpenAI surfaces |
| 14 | [Personal Data Layer](./14_PERSONAL_DATA_LAYER.md) | Power users | Working with personal data across supported surfaces |
...
| 25 | [Project Instruction Layers](./25_PROJECT_INSTRUCTION_LAYERS.md) | Power users | What belongs in each surface’s app fields, shared policy and native adapters, and how to keep the app-side fields from drifting unseen |
...
| 27 | [Independent Judgment](./27_INDEPENDENT_JUDGMENT.md) | All users | Getting an independently formed judgment: anchoring and agreement pressure, commit-then-reveal, blinded reconciliation, and why two model runs agreeing is not corroboration |
| 28 | [Second Brain](./28_SECOND_BRAIN.md) | Power users | A personal knowledge layer either assistant can read: the test for whether it is worth building, the four homes, inbox discipline, distilling for a conclusion rather than a summary, and the weekly review |
...
| 32 | [Action Authority](./32_ACTION_AUTHORITY.md) | All users | What an assistant may do without asking, classified by consequence rather than tool: four classes, standing approvals with scope and expiry, the proposal contract, the outbox and action log for unattended runs, and structural enforcement first |
...
| `tasks/setup-claude-md.md` | Interview → generate the shared policy and selected native adapters |
...
| `tasks/setup-policies.md` | Wire company policies into the `policies-validator` skill + shared policy with tiered handling and separately verified enforcement (T1 block / T2 alert / T3 guidance) |
...
| `tasks/setup-ignore-hygiene.md` | Audit Git hygiene and the selected surface’s context/permission controls |
| `tasks/setup-bootstrap-folder.md` | Create `bootstrap/` stubs for gitignored runtime state files |
| `tasks/tune-instruction-layers.md` | Review available app-side fields, shared policy, native adapters and source freshness |
...
| Task | What it does |
|------|-------------|
| `tasks/audit-claude-md.md` | Review the selected project instruction source — dead rules, missing sections, over-length |
...
| `tasks/analyze-project.md` | Analyze *another* assistant project (local or GitHub) against the full guide set → write a `CLUIDE_IMPROVEMENT_PLAN.md` into it (read-only, plan-only) |
...
**Claude Code:** install selected folders using Guide 03's Claude route. **Codex:** install selected folders into a supported `.agents/skills/` location using its route. **ChatGPT or conversational Claude:** use the product's supported skill installation, or supply the workflow as a source and request it explicitly. A source upload is not native installation. The `security-review` skill has separate OpenAI and Claude paths; its bundled shell hooks and `AGENT_STARTER_PACK` are Claude-specific.

| Skill | What it does |
|-------|-------------|
| `ai-assistant-setup` | Interactive setup coach — all guides bundled. Describe your surface and outcome; the coach selects the appropriate route. |
| `template-exporter` | Turns an assistant setup into a clean, shareable template |
| `cowork-optimizer` | Audits an assistant task for token efficiency and structural quality |
...
| `security-review` | Separate Claude and OpenAI security-review routes |
...
| Template | What it is |
|----------|-----------|
| `PROJECT_TEMPLATE/` | The default project layout: shared `AGENTS.md` and a `CLAUDE.md` adapter with a file map and an app-side mirror block, `Outputs/`, `Working/`, `_archive/`, plus the memory, intake, and routing blocks ready to keep or delete |

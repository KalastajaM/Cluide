...
- **Background sessions** — `claude --bg` starts a session that runs detached; `/goal` sets a standing objective for it (Claude checks in every 30 minutes while idle, tunable with `CLAUDE_CODE_GOAL_CHECKIN_MINUTES`). `claude agents` shows all running agent sessions.

...
- The MCP tools you use for visual verification — a browser MCP such as Claude in Chrome, or another preview/screenshot MCP if you have one installed — only the ones you actually rely on.
...
- [ ] `plans/` folder exists; use a simple feature-plan format (goal, checkboxed steps, file paths touched).

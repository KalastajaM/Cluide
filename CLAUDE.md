...
1. **Whole unit.** The branch is a complete change, not one batch of a larger sweep. A half-applied sweep is a stop however clean the diff looks — `main` is public, and a partial guide set is what a stranger clones.
...
5. **Downstream links.** If `CoWork Project Maintenance` is mounted, run its `scripts/check-cluide-links.sh`. If it is not mounted, report the check as skipped — never assume it passes.

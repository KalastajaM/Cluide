...
**The description field has a length cap.** The [Agent Skills specification](https://agentskills.io/specification) limits `description` to 1–1024 characters (and `name` to 64 lowercase letters, numbers and hyphens, matching the directory). Paths that enforce the spec — claude.ai uploads, the Skills API and `package_skill.py` ([code.claude.com/docs/en/skills](https://code.claude.com/docs/en/skills)) — can reject a longer one; a save/install path was observed doing exactly that on 2026-09-21, and the author had to shorten the description by hand after the save failed. Other hosts do not reject but cut: Claude Code truncates the combined `description` and `when_to_use` text at 1,536 characters in the skill listing, and Codex gives the whole initial skill list a budget (at most 2% of the model's context window, or 8,000 characters when that is unknown) and shortens descriptions first ([learn.chatgpt.com/docs/build-skills](https://learn.chatgpt.com/docs/build-skills)). The limit applies to the full `description:` value, not the line count in the YAML block. Being "pushy" (above) and staying under the cap pull the same direction: a description long enough to risk the cap is usually cramming detail that belongs in the skill body instead. Stay at or under 1024 so one SKILL.md installs everywhere, aim for well under it — a few hundred characters covers most skills — and check the actual character count before installing rather than finding the limit at save time. The frontmatter must also parse as YAML: a plain one-line description containing ": " breaks strict parsers, so use a folded `>` block (as above) or quote the value.
...
**Account skills in Claude Code:** from v2.1.275, Claude Code syncs the skills and plugins enabled on your claude.ai account into terminal sessions signed in with that account; opt out with `syncClaudeAiSkills: false` or `syncClaudeAiPlugins: false` ([Claude Code changelog](https://code.claude.com/docs/en/changelog)). A synced skill is downloaded from the account, not read from local files, so after editing one, update the account copy — editing a local file does not change what a synced session loads.

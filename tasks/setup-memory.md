...
Native Claude/ChatGPT/Codex memory stays platform-local and is not synchronized by this task; ChatGPT memory and local Codex memory (`~/.codex/memories/`, off by default) are themselves separate stores. Recurring jobs must explicitly load their own file state in a verified scheduler run. Assign one writer for shared memory or use isolated branches; record handoff date, revision, changed files, verification and pending work. Test recall in a fresh session on every supported surface before promising persistence.

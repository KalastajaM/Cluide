...
3. Create `wiki/source-notes/[slug].md`.
...
**MCP server config** — MCP servers are never configured in `settings.json` (see [Guide 05](./05_MCP_SERVERS.md)). In Claude Code, use `claude mcp add` or a project `.mcp.json`:
```json
{
  "mcpServers": {
    "qmd": {
...
├── source-notes/         ← 30 source summaries

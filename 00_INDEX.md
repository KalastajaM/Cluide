...
| Use one project from Claude and ChatGPT or Codex | Shared `AGENTS.md` + thin `CLAUDE.md` | [35](./35_DUAL_PLATFORM_PROJECTS.md) |

---

## Reading Tracks

The guide numbers are stable addresses, not a reading order — new guides get the next free number. Pick the track that matches what you're doing; each track lists its guides in reading order.
...
*Merged into [Guide 07, Part 9: Installing the Template](./07_TASK_LEARNING_GUIDE.md).* The installation steps (copy → fill in task name → wire into TASK.md) and the proposal-response protocol now live there; the template itself lives at [`templates/TASK_TEMPLATE/IMPROVEMENTS.md`](./templates/TASK_TEMPLATE/IMPROVEMENTS.md). The number 08 is retained as a stable address — the old file is a pointer stub.
...
Covers: when to split one project into linked projects (and when not to), how to split cleanly as a move-and-rewire, what to do when two independent projects grow an overlap, the single-owner principle and an ownership registry, cross-project linking conventions, drift reconciliation, how to change a project without breaking the ones that depend on it, and why the owner needs its own surface declaration because the registry lives with the coordinator.
...
**Use this when:** a deliverable is about to leave your context — going to a client, a counterparty, a new hire, or the public — or you are writing a high-stakes prompt you get roughly one attempt at.
...
### [29 — Specify Before You Rebuild](./29_SPEC_BEFORE_REBUILD.md)
`[Power user]` · `~15 min`

*The manoeuvre for an artefact that grew ad hoc and has started defending itself: state what it must do, then rebuild against that statement instead of patching further.*
...
### [35 — Dual-Platform Projects](./35_DUAL_PLATFORM_PROJECTS.md)
`[Power user]` · `~12 min`

*Running one project from Claude and from ChatGPT or Codex through a single shared policy, without pretending one product's mechanisms exist in the other.*

Covers: how coexistence differs from importing, when a second platform is worth its cost (and why Claude Code plus Cowork is not this), four homes for every rule and file (shared policy, native adapter, platform-specific rules, platform-local state), one shared `AGENTS.md` with a thin `CLAUDE.md` that imports it, app-side instructions and uploaded sources on each platform, capability gaps recorded as verified / none / untested rather than translated, native memory kept local, one scheduler owner per recurring job, one writer or separate branches plus a handoff record, a fresh-session check per surface, and one dated table holding every product-specific fact.

**Use this when:** a project will be worked on from both Claude and ChatGPT or Codex, a project's `CLAUDE.md` and `AGENTS.md` have started to disagree, or switching assistants lost state or ran a job twice.
...
**Use this when:** starting any new assistant project. Add further blocks from [`BLOCKS.md`](./templates/BLOCKS.md) as the project earns them.
...
### [AI-ASSISTANT_TEMPLATE/](./templates/AI-ASSISTANT_TEMPLATE/bootstrap/SETUP.md)
...
Go to **claude.ai → Skills → Upload skill** and drag in the `.zip` file you created.

...
<a id="what-changes-when-claude-is-the-reader"></a>

## What Changes When the Assistant Is the Reader
...
**`archive/`** — everything above, once dormant. The point of archiving rather than deleting is that the assistant can still search it on request while it stays out of the way of everything routine. Archive by moving the folder intact; do not flatten it.
...
Three patterns cover most of it. **Ask against the notes, not against the world** — "based on what is in `areas/property/`, what do I still owe on the renovation and what did I decide about the roof?" — where the value is that the answer is grounded in your record rather than in general knowledge. **Draft from the notes** — a document, a message, a position paper assembled from material you already distilled, which is the case where the up-front distillation pays for itself most visibly. **Resurface deliberately**: ask the assistant what in `resources/` bears on the thing you are currently working on. Old notes do not resurface by themselves, and the graph view that promises serendipity mostly delivers a pretty picture.
...
Thirty minutes, fixed slot, and it is the difference between a system and a pile. Most of it can be handed to the assistant; the judgment calls cannot.

The assistant does the mechanical pass: list what is in `inbox/`, flag notes in `projects/` whose project looks finished, find notes not touched in a year, find pairs of notes that appear to cover the same thing, find claims that contradict each other, and list any note whose `review:` date has passed. Ask for it as a report, not as edits.
...
| It is... | Home | Guide |
|---|---|---|
| A fact the assistant needs in order to behave correctly (your role, your preferences, a standing constraint) | Memory / profile | [04](./04_MEMORY_AND_PROFILE.md) |
...
**Notes in an app the assistant cannot read.** The most consequential structural mistake, and the most expensive to reverse once you have a thousand notes. Markdown on disk.
...
- [ ] Plain markdown in a folder the assistant can mount — not an app database

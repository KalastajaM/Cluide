...
| Google Calendar | Check Google's current per-minute Calendar quotas | Fine for personal use. Can be hit by tasks that poll calendars in a tight loop. |
| Slack | Slack Web API limits are per-method (e.g. ~1 message/second/channel for posting) | Read operations are more generous. Writing (posting messages) is throttled aggressively. |

...
| **Subagent** (Agent tool) | Nothing by default — see below; a Claude Code custom subagent with `omitClaudeMd: true` hides user, project and local CLAUDE.md | CLAUDE.md (unless `omitClaudeMd` is set; managed policy loads regardless), project instructions, folder read access, whatever you put in the brief. A `fork`-type subagent also inherits the entire conversation and the parent's auto memory. | One agent's tokens. Returns whatever its brief asks for. |
...
In Claude Code there is also a structural way to keep the project policy out of the reviewer. Since v2.1.271, `omitClaudeMd: true` in a custom or plugin subagent's frontmatter (or its `--agents` JSON) runs it "without user, project and local CLAUDE.md files; managed policy files still load" ([changelog](https://code.claude.com/docs/en/changelog)). That is a reviewer that has not read the project policy. It removes nothing else: the brief, folder read access and anything you pass it still reach it, so the deny-list below and the file-list check still apply. What comes back is labelled too: since v2.1.278 subagent results reach the main agent under a header marking them as subagent output, with the result indented, so text in a reviewed artefact that surfaces in the result cannot pass as the session's own instructions.

...
```
The `security-review` skill runs a version of this in Phase 0b.
...
**Important:** treat `.claudeignore` as advisory. Enforcement varies by product and version — recent Claude Code builds honour it for context exclusion, but treat it as hygiene, not a security boundary. Do not rely on it to protect sensitive files: the enforced mechanism for those is `permissions.deny` (with the Bash caveat below) plus folder scoping.
...
- **Minimum MCP access** — scope MCP servers per project so scheduled tasks only have the servers they actually need, not everything in your global config. `settings.json` never holds MCP config (see [Guide 05](./05_MCP_SERVERS.md)): use a project `.mcp.json` in Claude Code, or connector settings in Cowork.

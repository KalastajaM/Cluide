...
|---|---|---|---|---|---|
| Current salary band targets | job-search project | `Profile/targets.md` | budget project (read-only copy, dated) | 2026-08-01 | Active |
...
### The owner needs its own copy of the warning

Steps 1 and 2 above assume you can look up who depends on you. Often you cannot. The ownership registry lives with the project that coordinates the others, which is usually a consumer — so a session working inside the owner alone, with no consumer mounted, has no way to know a consumer exists at all. Owners are frequently worked on exactly that way: they are the stable project you open by itself to edit a definition.

So put a short declaration inside the owner, next to the thing it protects: which files, identifiers, and numbering are a public surface, and the rule that changing one means checking the registry and rewiring dependents in the same change. The registry stays with the coordinator; the *warning* has to live where the edit happens. Without it the impact check depends on whoever is editing happening to remember, which is not a control.

Two rules make that declaration cheap to honour:

- **Add, don't renumber.** Identifiers that consumers cite — dimension numbers, section numbers, task filenames — are addresses. Append new ones; retire rather than reuse; never shift the existing ones to make room.
- **Consumers reference, never restate derived values.** A count, a total, a "there are N of these" is a fact derived from the owner's content, and a consumer that writes the number down has forked it. The number then goes stale in a way no link check catches, because nothing is broken — it is merely wrong. Point at the source instead: *"the set defined in `<canonical location>`"*.

The second rule is the one that gets skipped, because restating a count feels like description rather than duplication. It is duplication, and it is the kind that survives every consistency check you have.

## Anti-patterns

- **Duplicating instead of pointing.** The single most common cause of cross-project drift.
- **Restating a derived value.** Copying a count, total, or version from the owner into a consumer. It cannot be caught by a link check, because nothing breaks — the number just quietly stops being true.
...
10. Keep a surface declaration inside the owner, because the registry lives with the coordinator and is invisible from a session that has only the owner open. Add rather than renumber, and never let a consumer restate a derived value like a count.

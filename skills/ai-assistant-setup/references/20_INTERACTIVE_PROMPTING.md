...
Each call can ask up to 4 questions, each with 2–4 options. Questions are single-select by default — one choice only, for mutually exclusive options:

> "Which environment should I deploy to?"
> - Staging
> - Production

Set `multiSelect: true` in the question definition to allow multiple choices, when options aren't mutually exclusive:

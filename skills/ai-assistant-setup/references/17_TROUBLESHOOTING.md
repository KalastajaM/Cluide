...
- [Something Broke After I Made a Change](#problem-something-broke-after-i-made-a-change)
- [My PreToolUse Hook Isn't Blocking Anything](#problem-my-pretooluse-hook-isnt-blocking-anything)
- [A Task Run Was Unexpectedly Expensive](#problem-a-task-run-was-unexpectedly-expensive)
...
1. **Read the evidence first.** Open `LAST_RUN.md` (if it exists) and read what actually happened (your task keeps either `RUN_LOG.md` or `LAST_RUN.md` — see [Guide 06](./06_TASK_EFFICIENCY_GUIDE.md)). The problem often explains itself here.
...
*Fix:* Force a local download of the affected files in Finder or Explorer, then retry.

**Recovering the current session:** ask for the outstanding list explicitly — *"List every file you produced this session that did not land in my folder, with the exact path each was meant to go to."* Then retry once the desktop is back, or save them from the chat yourself. Do this before ending the session; the session workspace does not survive it.

**Preventing it:** add the bridge guard to the project's instructions field ([Guide 25](./25_PROJECT_INSTRUCTION_LAYERS.md)). It checks the connection before the work rather than after, treats a file as delivered only once the write is confirmed, and ends the turn with a named list of what is outstanding instead of a request to download buried mid-reply. The rule has to live in the app-side field rather than CLAUDE.md, because a failed connection is precisely the case where CLAUDE.md may not be readable.
...
> "Read the current TASK.md and compare it to what you would expect based on LAST_RUN.md. Report any inconsistencies, and say explicitly if the two are consistent."

**To explain an improvement proposal:**
> "Read my IMPROVEMENTS.md and explain each pending proposal in plain language. For each one, tell me what it would change, why it was suggested, what could go wrong if I apply it, and what would have to be true for the proposal to be a mistake."

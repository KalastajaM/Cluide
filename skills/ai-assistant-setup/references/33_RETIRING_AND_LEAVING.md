...
| **Scheduled tasks** | Every task definition and every registration, wherever your surface keeps them — Cowork registrations, Claude Code Routines, ChatGPT scheduled tasks and Codex automations, and external schedulers or hooks | A disabled task is a stale reference with a switch; someone flips it |
...
| **The machine** | Codex configuration (`~/.codex/config.toml`), local Codex memory under `~/.codex/memories/`, and locally stored session state; Claude Code auto memory under `~/.claude/projects/`, `~/.claude/settings.json`, hooks, agents, `~/.claude.json`, the desktop app's state | Stays on the machine. On employer equipment it leaves with the laptop |
